...
OrderProvider extends ChangeNotifier {
  ...
  Map<int,CartModel> CartList = Map();
  ...
  double _distance = -1;
  Map<int,CartModel> get cartList => CartList;
  ...
  void choosePhotoFromCamera(int index, BuildContext context) async {
    ...
    if (pickedFile != null) {
      ...
      CartList[index] =CartModel(1, "_image", "_name", 1, "", 0, 0, null, 0, 0, 0, "", 0) ;
      CartList[index].prescriptionImage=_file.path;
      print("save in this index "+index.toString());
    } else {
      print('No image selected.');
    }
    notifyListeners();
  }
  void choosePhoto(int index, BuildContext context) async {
  ...
  void pickImage() async {
    _data = await picker.getImage(source: ImageSource.gallery);
    notifyListeners();
  }
  ...
  bool exists( int index){
    ...
    return cartList.containsKey(index);
  }
  ...
  Future<void> placeOrder(PlaceOrderBody placeOrderBody, Function callback, String token) async {
    PickedFile _imageFile = data;
    _isLoading = true;
    notifyListeners();
    print(placeOrderBody.toJson());
    ApiResponse apiResponse = await orderRepo.placeOrder(placeOrderBody, _imageFile, token);
    _isLoading = false;
    if (apiResponse.response != null && apiResponse.response.statusCode == 200) {
