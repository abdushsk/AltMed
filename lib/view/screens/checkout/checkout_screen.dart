                        ...
                        child: CustomButton(
                          ...
                          onPressed: () async {
                            if (!_selfPickup && order.addressIndex == -1) {
                              ...
                              OrderProvider _cartList2 = Provider.of<OrderProvider>(context, listen: false);
                              ...
                              String encoded = (_cartList2.exists(0)?_cartList2.CartList[0].prescriptionImage:"");

                              if (encoded!=""){
                                File imageFile = File(encoded);
                                Uint8List imagebytes = await imageFile.readAsBytes(); //convert to bytes
                                encoded = "data:image/png;base64," + cv.base64.encode(imagebytes); //convert bytes to base64 string
                                print("zupp "+encoded.length.toString());
                              }
                              order.placeOrder(PlaceOrderBody(
                                ...
                                deliveryAddressId: !_selfPickup ? Provider.of<LocationProvider>(context, listen: false)
                                    .addressList[order.addressIndex].id : 0, distance: _selfPickup ? 0 : order.distance, image: (encoded),
                              ...
                              ), _callback, Provider.of<AuthProvider>(context, listen: false).getUserToken(),);
                            }
                          },
                        ),
                      ),
